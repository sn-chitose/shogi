...
public class BoardManager : MonoBehaviour
{
    ...
    public void TryAndDrop(int x, int y)
    {
        ...
        {
            KifuManager.instance.AddMove(SelectedPiece, x, y, true);
            DropPiece(x, y);
        }
    }
    ...
    public void TryAndCapture(Piece toCapture)
    {
        ...
        {
            if (MoveManager.IsPromotionForced(SelectedPiece, (int)toCapture.transform.position.x, (int)toCapture.transform.position.y))
            {
                KifuManager.instance.AddMove(SelectedPiece, toCapture, true);
                CapturePiece(toCapture, true);
            }
            else if (MoveManager.IsPromotable(SelectedPiece, (int)toCapture.transform.position.x, (int)toCapture.transform.position.y))
            {
                ...
                {
                    KifuManager.instance.AddMove(SelectedPiece, toCapture, false);
                    CapturePiece(toCapture);
                });
            }
            else
            {
                KifuManager.instance.AddMove(SelectedPiece, toCapture);
                CapturePiece(toCapture);
            }
        }
    }
    ...
    public void TryAndMove(int x, int y)
    {
        if (SelectedPiece.LegalMoves.Contains(new Vector2Int(x, y)))
        {
            if (MoveManager.IsPromotionForced(SelectedPiece, x, y))
            {
                KifuManager.instance.AddMove(SelectedPiece, x, y, false, true);
                MovePiece(x, y, true);
            }
            else if (MoveManager.IsPromotable(SelectedPiece, x, y))
            {
                ShowPromotionMessage(() =>
                {
                    KifuManager.instance.AddMove(SelectedPiece, x, y, false, true);
                ...
                {
                    KifuManager.instance.AddMove(SelectedPiece, x, y, false, false);
                    MovePiece(x, y);
                });
            }
            else
            {
                KifuManager.instance.AddMove(SelectedPiece, x, y);
                MovePiece(x, y);
            }
        }
    }
    ...
    public void CapturePiece(Piece toCapture, bool promote = false)
    {
        ...
        seq.Join(toCapture.transform.DOMove(BoardGrid.GetPositionWhenCaptured(toCapture, SelectedPiece.IsPlayer2()), 0.5f));
        seq.Join(toCapture.transform.DORotate(new Vector3(0, 0, SelectedPiece.IsPlayer2() ? 180 : 0), 0.5f, RotateMode.FastBeyond360));

        if (toCapture.Promoted)
        {
            var scaling = DOTween.Sequence();
            scaling.AppendInterval(0.2f);
            scaling.Append(toCapture.transform.DOScaleX(0f, 0.05f)
                .OnComplete(() => toCapture.Promoted = false));
            scaling.Append(toCapture.transform.DOScaleX(1f, 0.05f));
            seq.Join(scaling);
        }
    ...
    public void MovePiece(int x, int y, bool promote = false)
    {
        ...
        var seq = DOTween.Sequence();
        seq.Join(moving.transform.DOMove(new Vector3(x, y, 0f), 0.5f));

        if (promote)
        {
            var scaling = DOTween.Sequence();
            scaling.AppendInterval(0.2f);
            scaling.Append(moving.transform.DOScaleX(0f, 0.05f)
                .OnComplete(() => moving.Promoted = true));
            scaling.Append(moving.transform.DOScaleX(1f, 0.05f));
            seq.Join(scaling);
        }
